...
→ Evidence Matcher 用 graph 中的证据组匹配原话
→ Graph Evaluator 评估节点、关系、上下文转场和四维分数
→ Knowledge Judge 核验知识事实
→ Constraint Judge 核验限制与合规边界
...
## 11. 第十步：Graph Evaluator 评估节点、关系、上下文和分数
...
`GraphEvaluator` 是本地评估主干。它会生成四类核心结果。
...
## 12. 第十一步：Knowledge Judge 核验知识事实
...
## 13. 第十二步：Constraint Judge 核验限制边界
