...
def compile_state_graph(raw_graph: dict[str, Any], legacy_dialogue_root: str | Path | None = None, *, allow_legacy_exact_patterns: bool = False) -> dict[str, Any]:
    ...
    try:
        from .schema_final_tightener import tighten_graph_contracts
        graph = tighten_graph_contracts(graph)
    except Exception:
        pass
    # Offline graphs generated by an unstable LLM may contain an exploded hard
    # table (dozens of generic rows) or soft-quality rows leaked into hard.
    # This is structural sanitation only: it never creates new business rules.
    try:
        hard_rows = graph.get("hard_constraint_table") or []
        hard_soft_leak = any(isinstance(x, dict) and str(x.get("constraint_kind") or "").lower() == "fuzzy_quality" for x in hard_rows)
        if isinstance(hard_rows, list) and (len(hard_rows) > 20 or hard_soft_leak):
            from .schema_atomic_pipeline import sanitize_constraint_tables
            graph = sanitize_constraint_tables(graph)
    except Exception:
        pass
    _merge_constraint_tables(graph)
    _normalize_nodes(graph)
    _clear_node_atom_fact_flags(graph)
    _normalize_edges_and_relation_groups(graph)
    _normalize_knowledge(graph)
    _refresh_table_binding_metadata(graph)
    ...
    graph["metadata"]["schema_compiler"] = "local_compile_normalize_antileak"
    return graph
...
def _merge_constraint_tables(graph: dict[str, Any]) -> None:
    """Merge hard/soft constraint sources into one de-duplicated executable table.

    hard_constraint_table and soft_constraint_table remain as source/audit
    tables.  constraint_table is the executor-facing table.  The same rule may
    appear in both after repeated compile/lint cycles, so this function is
    ...
    for key, enforcement, default_kind in (
        ...
        for item in graph.get(key, []) or []:
            ...
            merged.append(rule)
    for item in graph.get("constraint_table", []) or []:
        if not isinstance(item, dict):
            continue
        rule = copy.deepcopy(item)
        rule.setdefault("enforcement", "soft" if rule.get("constraint_kind") == "fuzzy_quality" else "hard")
        rule.setdefault("constraint_kind", "fuzzy_quality" if rule.get("enforcement") == "soft" else rule.get("constraint_type") or "semantic_object")
        merged.append(rule)

    seen: set[tuple[str, str, str]] = set()
    deduped: list[dict[str, Any]] = []
    for idx, rule in enumerate(merged, start=1):
        cid = str(rule.get("id") or rule.get("constraint_id") or f"constraint_{idx}")
        enforcement = str(rule.get("enforcement") or ("soft" if rule.get("constraint_kind") == "fuzzy_quality" else "hard"))
        kind = str(rule.get("constraint_kind") or rule.get("constraint_type") or ("fuzzy_quality" if enforcement == "soft" else "semantic_object"))
        ...
        deduped.append(rule)
    graph["constraint_table"] = deduped

def _normalize_nodes(graph: dict[str, Any]) -> None:
    normalized_nodes: list[dict[str, Any]] = []
    for nidx, node in enumerate(graph.get("nodes", []) or [], start=1):
        if not isinstance(node, dict):
            text = str(node or "").strip()
            if not text:
                continue
            node = {"id": f"node_{nidx}", "name": text, "requirements": [text]}
        else:
            node = copy.deepcopy(node)
        # LLM graph-core prompts sometimes emit node_id while the runtime
        # schema uses id.  Normalize once at compile time and preserve node_id
        # for reports/table bindings so offline graphs from both contracts run.
        raw_node_id = node.get("id") or node.get("node_id") or f"node_{nidx}"
        node["id"] = str(raw_node_id)
        node.setdefault("node_id", node["id"])
        node.setdefault("type", node.get("node_type") or node.get("state_role") or "process")
        node.setdefault("node_type", node.get("type") or "process")
        _normalize_activation_field(node)
        node.setdefault("aliases", [])
        node["aliases"] = _dedupe([*node.get("aliases", []), node.get("id", ""), node.get("node_id", ""), node.get("name", "")])
        activation_terms = _activation_terms(node)
        reqs = _normalize_requirement_items(node.get("requirements") or [], node, source="requirements")
        if not reqs and node.get("evidence_groups"):
            reqs = []
            for gidx, g in enumerate(_normalize_evidence_groups(node.get("evidence_groups"), {"id": node.get("id", "node"), "text": node.get("name", "")}), start=1):
                reqs.append(
                    {
                        "id": g.get("id") or f"{node.get('id','node')}.r{gidx}",
                        "text": g.get("description") or g.get("text") or node.get("name") or "",
        ...
        for idx, req in enumerate(node.get("requirements", []) or [], start=1):
            ...
            _normalize_positive_object_requirement(req)
            groups = _normalize_evidence_groups(req.get("evidence_groups") or [], req)
            if not groups and req.get("patterns") is not None:
                groups = [
                    {
                        "id": f"{req['id']}.evidence",
                        "description": req.get("text") or "",
                        "patterns": _normalize_pattern_list(req.get("patterns") or [], default_speaker="assistant"),
            ...
            for gidx, group in enumerate(groups, start=1):
                ...
                group["patterns"] = _normalize_pattern_list(group.get("patterns") or group.get("pattern") or [], default_speaker="assistant")
...
def _normalize_requirement_items(value: Any, node: dict[str, Any], *, source: str = "requirements") -> list[dict[str, Any]]:
    """Coerce LLM shorthand requirements into dictionaries.

    LLM sometimes emits requirements as plain strings, lists of terms, or a
    mapping instead of the full object form.  The compiler must be tolerant here
    because this is still graph-supplied schema data, not executable code logic.
    This helper wraps shorthand into a neutral positive-object requirement so
    downstream normalization can proceed without crashing.
    """
    if value is None:
        return []
    if isinstance(value, dict):
        # Single requirement object.
        if any(k in value for k in ("id", "text", "description", "name", "positive_object", "patterns", "evidence_groups", "primary_elements")):
            return [copy.deepcopy(value)]
        # Mapping shorthand: {requirement_id: requirement_text_or_object}.
        out: list[dict[str, Any]] = []
        for key, item in value.items():
            if isinstance(item, dict):
                req = copy.deepcopy(item)
                req.setdefault("id", str(key))
            else:
                text = str(item or key or "").strip()
                req = {"id": str(key), "text": text}
            out.append(req)
        return out
    if isinstance(value, (str, int, float)):
        text = str(value).strip()
        return [{"text": text, "positive_object": {"object_type": "positive_object", "description": text, "surface_forms": [text]}}] if text else []
    if isinstance(value, (list, tuple, set)):
        ...
        for item in value:
            if isinstance(item, dict):
                ...
                vals = _string_list(item)
                if vals:
                    text = " / ".join(vals)
                    out.append({"text": text, "positive_object": {"object_type": "positive_object", "description": text, "surface_forms": vals}})
            else:
                text = str(item or "").strip()
                if text:
                    out.append({"text": text, "positive_object": {"object_type": "positive_object", "description": text, "surface_forms": [text]}})
        return out
    return []


def _normalize_evidence_groups(value: Any, req: dict[str, Any]) -> list[dict[str, Any]]:
    """Coerce evidence_groups shorthand into dictionaries."""
    if value is None:
        return []
    if isinstance(value, dict):
        groups = [copy.deepcopy(value)]
    elif isinstance(value, (str, int, float)):
        text = str(value).strip()
        groups = [{"description": text, "patterns": _normalize_pattern_list(text, default_speaker="assistant")}] if text else []
    elif isinstance(value, (list, tuple, set)):
        groups = []
        for item in value:
            if isinstance(item, dict):
                groups.append(copy.deepcopy(item))
            else:
                text = str(item or "").strip()
                if text:
                    groups.append({"description": text, "patterns": _normalize_pattern_list(text, default_speaker="assistant")})
    else:
        return []
    for gidx, group in enumerate(groups, start=1):
        group.setdefault("id", f"{req.get('id','requirement')}.g{gidx}")
        group.setdefault("description", group.get("text") or req.get("text") or "")
        group.setdefault("min_hits", group.get("min_hits", 1))
        group.setdefault("required", group.get("required", True))
        group.setdefault("weight", group.get("weight", 1.0))
        group["patterns"] = _normalize_pattern_list(group.get("patterns") or group.get("pattern") or group.get("any") or group.get("all") or [], default_speaker="assistant")
    return groups


def _normalize_positive_object_requirement(req: dict[str, Any]) -> None:
    """Normalize the target contract for node requirements.

    A node requirement is a positive-object check: the schema declares the
    target action/state to be supported, and runtime scans dialogue evidence for
    that target.  This normalizer is structural only. It wraps model shorthand
    into a dict and never injects business terms from code.
    """
    obj = req.get("positive_object") or req.get("target_object") or {}
    if isinstance(obj, str):
        obj = {"object_type": "positive_object", "description": obj, "surface_forms": [obj]} if obj.strip() else {}
    elif isinstance(obj, list):
        obj = {"object_type": "positive_object", "surface_forms": [str(x) for x in obj if str(x or "").strip()]}
    elif not isinstance(obj, dict):
        obj = {}
    if obj:
        obj.setdefault("object_type", obj.get("type") or "positive_object")
        obj.setdefault("description", req.get("text") or req.get("description") or req.get("name") or "")
        # Normalize common target fields into arrays so PositiveObjectScanner can
        # execute them uniformly. Values remain graph-supplied.
        for key, value in list(obj.items()):
            if key.endswith(("_terms", "_aliases", "_markers", "_objects", "_actions", "_attributes", "_values")) or key in {"surface_forms", "semantic_equivalents", "examples", "example_phrases", "evidence_phrases"}:
                obj[key] = _string_list(value)
        if "slot_groups" in obj and isinstance(obj["slot_groups"], dict):
            obj["slot_groups"] = [{"name": str(k), "any": _string_list(v)} for k, v in obj["slot_groups"].items()]
        req["positive_object"] = obj
    elif "positive_object" in req:
        req["positive_object"] = {}
...
def _string_list(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        ...
        for key in ("any", "terms", "aliases", "values", "surface_forms"):
            out.extend(_string_list(value.get(key)))
...
def _normalize_pattern_list(value: Any, *, default_speaker: str = "assistant") -> list[dict[str, Any]]:
    """Coerce LLM's occasional shorthand pattern output into schema dicts.

    This is a structural guard only. It does not add domain words; it only wraps
    strings/lists/dicts already returned in the schema so downstream compiler,
    linter and evaluator do not crash on valid-but-shorthand model output.
    """
    if value is None:
        return []
    if isinstance(value, dict):
    ...
    if isinstance(value, str):
        text = value.strip()
        return [{"speaker": default_speaker, "any": [text]}] if text else []
    if isinstance(value, (list, tuple)):
        ...
        for item in value:
            if isinstance(item, dict):
                ...
                text = item.strip()
                if text:
                    scalar_terms.append(text)
            elif isinstance(item, (int, float)):
                scalar_terms.append(str(item))
        if scalar_terms:
            out.append({"speaker": default_speaker, "any": scalar_terms})
        return out
    return []


def _normalize_activation_field(node: dict[str, Any]) -> None:
    activation = node.get("activation")
    if isinstance(activation, dict):
        mode = str(activation.get("mode") or "always").strip() or "always"
        patterns = _normalize_pattern_list(activation.get("patterns"), default_speaker="user")
        trigger_object = _normalize_trigger_object(activation.get("trigger_object") or activation.get("target_object") or activation.get("positive_object") or {})
        # Preserve the clean-prompt contract fields.  Older compiler code kept
        # only trigger_object/patterns, which erased trigger_hint and
        # trigger_groups before the quality gate and offline evaluator could see
        # them.  The evaluator owns the execution policy; these fields are only
        # graph-supplied semantic material.
        kept = {"mode": mode, "patterns": patterns, "trigger_object": trigger_object}
        for key in (
            "trigger_hint", "description", "hint",
            "trigger_groups", "trigger_element_groups", "element_groups",
            "primary_elements", "secondary_elements", "secondary_pools", "match_policy",
            "element_anchor_id",
        ):
            if key in activation and activation.get(key) not in (None, [], {}):
                kept[key] = copy.deepcopy(activation.get(key))
        node["activation"] = kept
        return
    if isinstance(activation, str):
        text = activation.strip()
        lower = text.lower()
        if lower in {"always", "start", "required", "must"} or text in {"默认", "始终", "必做", "常规"}:
            node["activation"] = {"mode": "always", "patterns": [], "trigger_object": {}}
        elif lower in {"optional", "faq"} or text in {"可选", "非必做"}:
            node["activation"] = {"mode": "optional", "patterns": [], "trigger_object": {}}
        elif lower in {"user_triggered", "condition", "conditional"} or text in {"用户触发", "条件触发"}:
            node["activation"] = {"mode": "user_triggered", "patterns": [], "trigger_object": {}}
        else:
            node["activation"] = {"mode": "user_triggered", "patterns": [{"speaker": "user", "any": [text]}], "trigger_object": _normalize_trigger_object(text)}
        return
    if isinstance(activation, (list, tuple)):
        node["activation"] = {"mode": "user_triggered", "patterns": _normalize_pattern_list(activation, default_speaker="user"), "trigger_object": _normalize_trigger_object(activation)}
        return
    node["activation"] = {"mode": "always", "patterns": [], "trigger_object": {}}


def _normalize_trigger_object(value: Any) -> dict[str, Any]:
    """Normalize node/constraint trigger target without adding domain terms."""
    if value is None:
        return {}
    if isinstance(value, str):
        text = value.strip()
        return {"object_type": "trigger_object", "description": text, "surface_forms": [text]} if text else {}
    if isinstance(value, (list, tuple, set)):
        vals = _string_list(value)
        return {"object_type": "trigger_object", "surface_forms": vals} if vals else {}
    if not isinstance(value, dict):
        return {}
    obj = dict(value)
    if not obj:
        return {}
    obj.setdefault("object_type", obj.get("type") or "trigger_object")
    obj.setdefault("description", obj.get("name") or obj.get("text") or "")
    for key, val in list(obj.items()):
        if key.endswith(("_terms", "_aliases", "_markers", "_objects", "_actions", "_attributes", "_values")) or key in {"surface_forms", "semantic_equivalents", "examples", "example_phrases", "evidence_phrases"}:
            obj[key] = _string_list(val)
    if "slot_groups" in obj and isinstance(obj["slot_groups"], dict):
        obj["slot_groups"] = [{"name": str(k), "any": _string_list(v)} for k, v in obj["slot_groups"].items()]
    return obj
...
def _normalize_relation_label(value: Any, default: str = "before") -> str:
...
def _normalize_edges_and_relation_groups(graph: dict[str, Any]) -> None:
    """Normalize state-graph transitions without flattening their semantics."""
    normalized_edges: list[dict[str, Any]] = []
    for idx, raw in enumerate(graph.get("edges", []) or [], start=1):
        ...
        etype = _normalize_relation_label(edge.get("type") or edge.get("edge_type") or edge.get("relation") or "before", "before")
        edge["type"] = etype
        edge["relation"] = etype
        edge.setdefault("id", edge.get("edge_id") or f"edge_{idx:02d}")
        if isinstance(edge.get("condition"), dict):
            c = edge["condition"]
            edge["condition"] = str(c.get("description") or c.get("text") or c)
        else:
            edge["condition"] = str(edge.get("condition") or "")
        if isinstance(edge.get("terminal_effect"), dict):
            pass
        elif isinstance(edge.get("effect"), dict):
            edge["terminal_effect"] = copy.deepcopy(edge.get("effect"))
        normalized_edges.append(edge)
    graph["edges"] = normalized_edges

    groups: list[dict[str, Any]] = []
    for idx, raw in enumerate(graph.get("relation_groups", []) or [], start=1):
        if not isinstance(raw, dict):
            continue
        group = copy.deepcopy(raw)
        group.setdefault("id", group.get("group_id") or f"relation_group_{idx:02d}")
        group.setdefault("group_id", group.get("id"))
        gtype = _normalize_relation_label(group.get("type") or group.get("group_type") or group.get("relation") or group.get("relation_type") or "all_of", "all_of")
        group["type"] = gtype
        group["relation"] = gtype
        groups.append(group)
    graph["relation_groups"] = groups

def _normalize_knowledge(graph: dict[str, Any]) -> None:
    normalized_items: list[dict[str, Any]] = []
    for idx, raw_item in enumerate(graph.get("knowledge_table", []) or [], start=1):
        if isinstance(raw_item, dict):
            ...
            item = {
                ...
                "claims": [{
                    "id": f"knowledge_{idx}.claim",
                    "name": text,
                    "support_patterns": _normalize_pattern_list(text, default_speaker="assistant"),
                    "refute_patterns": [],
                    "aliases": [text],
                }],
            }
        ...
        if item.get("claims"):
            ...
            for cidx, claim in enumerate(item.get("claims", []) or [], start=1):
                if isinstance(claim, dict):
                    ...
                    if not text:
                        continue
                    claim = {
                        "id": f"{item.get('id','knowledge')}.claim{cidx}",
                        "name": text,
                        "support_patterns": _normalize_pattern_list(text, default_speaker="assistant"),
                        "refute_patterns": [],
                    }
                ...
                claim["support_patterns"] = _normalize_pattern_list(claim.get("support_patterns") or [], default_speaker="assistant")
                claim["refute_patterns"] = _normalize_pattern_list(claim.get("refute_patterns") or claim.get("conflict_patterns") or [], default_speaker="assistant")
                if claim.get("claim_patterns") is not None:
                    claim["claim_patterns"] = _normalize_pattern_list(claim.get("claim_patterns") or [], default_speaker="assistant")
                ...
                item["claims"].append(
                    {
                        ...
                        "support_patterns": _normalize_pattern_list(item.get("support_patterns") or [], default_speaker="assistant"),
                        "refute_patterns": _normalize_pattern_list(item.get("refute_patterns") or item.get("conflict_patterns") or [], default_speaker="assistant"),
        ...
        _normalize_value_check_wrong_groups(item)
        _drop_identical_wrong_groups(item)
        normalized_items.append(item)
    graph["knowledge_table"] = normalized_items
...
def _clean_claim_refute_patterns(item: dict[str, Any]) -> None:
    ...
    LLM/schema binding may accidentally copy positive coverage descriptions
...
def _value_check_has_declared_comparable(item: dict[str, Any]) -> bool:
    """Return True when a knowledge row declares comparable facts in value_check.

    Numeric/time/money/count facts are judged by value_check, not by enumerated
    wrong_groups.  This is structural and graph-driven: it only looks at the
    row's own value_check declaration and does not inject task vocabulary.
    """
    vc = item.get("value_check")
    if not isinstance(vc, dict):
        return False
    candidates: list[Any] = [vc.get("expected_value"), vc.get("expected"), vc.get("normalized_expected")]
    ...
    if isinstance(raw_checks, list):
        for check in raw_checks:
            if isinstance(check, dict):
                candidates.extend([check.get("expected_value"), check.get("expected"), check.get("normalized_expected")])
    return any(str(x or "").strip() for x in candidates)


def _groups_signature(groups: Any) -> set[str]:
    sig: set[str] = set()
    ...
    for g in groups:
        ...
        for e in g.get("elements") or []:
            if isinstance(e, dict):
                val = _norm_value(e.get("value"))
                if val:
                    sig.add(val)
    return sig

def _drop_identical_wrong_groups(item: dict[str, Any]) -> None:
    w_sig = _groups_signature(item.get("wrong_groups"))
    c_sig = _groups_signature(item.get("correct_groups"))
    if w_sig and c_sig and w_sig <= c_sig:
        item["wrong_groups"] = []
        item.pop("negative_groups", None)

def _clear_node_atom_fact_flags(graph: dict[str, Any]) -> None:
    for node in graph.get("nodes", []) or []:
        ...
        for atom in node.get("atoms", []) or []:
            if not isinstance(atom, dict):
                continue
            for group in atom.get("element_groups", []) or []:
...
def _normalize_value_check_wrong_groups(item: dict[str, Any]) -> None:
    """Prevent numeric/time value_check rows from firing wrong_groups by topic.

    Long model outputs sometimes put the opposite numeric value in wrong_groups.
    If that opposite value is not marked as fact or is missed by surface form
    normalization, the local matcher can mark a correct sentence as conflict just
    because the selector topic was hit.  The project contract says comparable
    facts must be judged by value_check, so we drop wrong_groups on such rows.
    """
    if not _value_check_has_declared_comparable(item):
        return
    if item.get("wrong_groups"):
        item["wrong_groups"] = []
    if item.get("negative_groups") and str(item.get("judge_type") or "").startswith("element_fact"):
        # Knowledge rows should not use negative_groups for comparable value
        # conflicts; hard constraints still use negative_groups in constraint_table.
        item["negative_groups"] = []

def _refresh_table_binding_metadata(graph: dict[str, Any]) -> None:
    """Recompute knowledge node/atom binding warnings after node-id normalization.

    Some previously generated graphs already contain stale warnings caused by the
    graph-core pass using ``node_id`` while the local compiler expected ``id``.
    After normalizing both forms, keep only real binding problems.
    """
    node_ids = {str(n.get("id") or n.get("node_id")) for n in graph.get("nodes", []) or [] if isinstance(n, dict) and (n.get("id") or n.get("node_id"))}
    atoms_by_node: dict[str, set[str]] = {}
    for node in graph.get("nodes", []) or []:
        if not isinstance(node, dict):
            continue
        ...
        if not nid:
            continue
        atoms_by_node[nid] = {str(a.get("id") or a.get("atom_id")) for a in node.get("atoms", []) or [] if isinstance(a, dict) and (a.get("id") or a.get("atom_id"))}
    ...
    for item in graph.get("knowledge_table", []) or []:
        ...
        if not raw_atom_ids and isinstance(item.get("atoms"), list):
            raw_atom_ids = [a.get("id") or a.get("atom_id") for a in item.get("atoms") or [] if isinstance(a, dict)]
        atom_ids = [str(x) for x in raw_atom_ids if str(x or "").strip()]
        if nid and atom_ids:
            allowed = atoms_by_node.get(nid, set())
            bad_atoms = [x for x in atom_ids if x not in allowed]
            if bad_atoms:
                warnings.append({"type": "knowledge_bad_atom_binding", "id": item.get("id"), "node_id": nid, "atom_ids": bad_atoms})
    meta = graph.setdefault("metadata", {})
    meta.pop("table_binding_warnings", None)
    if warnings:
        meta["table_binding_warnings"] = warnings
...
def _normalize_constraints(graph: dict[str, Any]) -> None:
    for rule in graph.get("constraint_table", []) or []:
        ...
        rule.setdefault("constraint_kind", "fuzzy_quality" if enforcement == "soft" else rule.get("constraint_type") or "semantic_object")
        rule.setdefault("trigger_policy", "global_style" if enforcement == "soft" else ("requires_user_trigger" if rule.get("trigger") else "self_sufficient"))
        rule.setdefault("aliases", [])
        rule["aliases"] = _dedupe([*rule.get("aliases", []), rule.get("id", ""), rule.get("name", "")])
        rule.setdefault("negative_object", {})
        rule["trigger_object"] = _normalize_trigger_object(rule.get("trigger_object") or rule.get("trigger_target") or {})
        rule.setdefault("detection_scope", {})
        rule.setdefault("verdict_logic", "")
        rule.setdefault("prohibited", [])
        rule.setdefault("safe_context", [])
        rule.setdefault("unresolved", rule.get("grey_zone", []))
        _normalize_negative_object_rule(rule)
        _ensure_constraint_violation_scope(rule)




def _to_pattern_list_from_texts(values: Any, *, speaker: str = "assistant", reason: str = "") -> list[dict[str, Any]]:
    texts: list[str] = []
    ...
    if not texts:
        return []
    pat: dict[str, Any] = {"speaker": speaker, "any": texts}
    if reason:
        pat["reason"] = reason
    return [pat]


def _normalize_negative_object_rule(rule: dict[str, Any]) -> None:
    """Compile negative_object hints into executable pattern fields.

    This function is structural: it only reuses text supplied by the graph.  It
    does not inject domain words.  The aim is to make the new hard table runnable
    by the existing matcher while preserving the method distinction: hard rules
    scan for negative objects; soft rules are carried but not enforced locally.
    """
    if str(rule.get("enforcement") or "hard") == "soft":
        rule.setdefault("soft_rule", rule.get("evaluation_basis") or {})
        rule.setdefault("prohibited", [])
        rule.setdefault("safe_context", [])
        rule.setdefault("trigger", [])
        return

    neg = rule.get("negative_object") or {}
    if not isinstance(neg, dict):
        neg = {"description": str(neg)} if str(neg or "").strip() else {}
    rule["negative_object"] = neg
    if not rule.get("trigger_object"):
        trigger_context = str(neg.get("trigger_context") or "").strip()
        if trigger_context and str(rule.get("trigger_policy") or "") in {"requires_user_trigger", "context_dependent"}:
            rule["trigger_object"] = _normalize_trigger_object({
                "object_type": "constraint_trigger",
                "description": trigger_context,
                "surface_forms": [trigger_context],
            })

    # Self-sufficient phrase/semantic objects: direct assistant-side scan.
    if not rule.get("prohibited"):
        direct_terms: list[str] = []
        direct_terms.extend(str(x or "").strip() for x in neg.get("forbidden_surface_forms") or [] if str(x or "").strip())
        direct_terms.extend(str(x or "").strip() for x in neg.get("semantic_equivalents") or [] if str(x or "").strip())
        forbidden_action = str(neg.get("forbidden_action") or "").strip()
        if forbidden_action:
            direct_terms.append(forbidden_action)
        if direct_terms:
            pat = {"speaker": "assistant", "any": _dedupe(direct_terms), "reason": "命中 hard_constraint_table.negative_object"}
            if str(rule.get("trigger_policy") or "self_sufficient") == "self_sufficient":
                pat["self_sufficient"] = True
            else:
                pat["requires_trigger"] = True
            rule["prohibited"] = [pat]

    # Contextual objects: generate user trigger/safe patterns from schema text.
    if not rule.get("trigger"):
        trigger_text = str(neg.get("trigger_context") or "").strip()
        if trigger_text and str(rule.get("trigger_policy") or "") in {"requires_user_trigger", "context_dependent"}:
            rule["trigger"] = _to_pattern_list_from_texts([trigger_text], speaker="user", reason="触发 hard 负向对象扫描上下文")
    if not rule.get("safe_context"):
        safe_text = str(neg.get("required_safe_action") or "").strip()
        if safe_text:
            rule["safe_context"] = _to_pattern_list_from_texts([safe_text], speaker="assistant", reason="触发后安全处理动作")

    # Preserve explicit detection scope defaults.
    scope = dict(rule.get("detection_scope") or {})
    scope.setdefault("speaker", "assistant")
    scope.setdefault("window", "whole_dialogue" if not rule.get("trigger") else "after_trigger")
    scope.setdefault("allow_multiple", bool(rule.get("allow_multiple", False)))
    rule["detection_scope"] = scope

def _ensure_constraint_violation_scope(rule: dict[str, Any]) -> None:
    """Build a schema-side violation_scope when LLM omitted it.
    ...
    Newer LLM prompts should output violation_scope directly; this migration
    keeps older graphs executable under the same schema-driven executor.
    """
    scope = dict(rule.get("violation_scope") or {})
    neg = rule.get("negative_object") or {}
    risk_targets = neg.get("risk_targets") if isinstance(neg, dict) else []
    forbidden_action = neg.get("forbidden_action") if isinstance(neg, dict) else ""
    required_safe_action = neg.get("required_safe_action") if isinstance(neg, dict) else ""
    trigger_context = neg.get("trigger_context") if isinstance(neg, dict) else ""
    scope.setdefault("protected_objects", _to_pattern_list_from_texts(risk_targets, speaker="assistant") or _derive_scope_objects(rule))
    scope.setdefault("forbidden_actions", _to_pattern_list_from_texts([forbidden_action], speaker="assistant") or _derive_scope_actions(rule.get("prohibited") or [], fallback_name=rule.get("name") or rule.get("description") or ""))
    scope.setdefault("safe_actions", _to_pattern_list_from_texts([required_safe_action], speaker="assistant") or _derive_scope_actions(rule.get("safe_context") or [], fallback_name=""))
    scope.setdefault("ambiguous_zone", rule.get("unresolved") or rule.get("grey_zone") or [])
    scope.setdefault("trigger_scope", _to_pattern_list_from_texts([trigger_context], speaker="user") if trigger_context else [])
            ...
            out.append(
                {
                    ...
                    # Answer-key spans are intentionally not collected here.
                    # They must not be compiled into executable patterns or used
                    # to route arbitration.
                    "wrong_statement": "",
...
def _synthesize_missing_knowledge_targets(graph: dict[str, Any], legacy: list[dict[str, Any]]) -> None:
    """Create schema-side knowledge checks for dataset target IDs missing from LLM.

    LLM sometimes represents a business fact only as a requirement, while
...
def _legacy_hint(item: dict[str, Any]) -> str:
    ...
    bind legacy injected_error ids to whatever IDs LLM generated this run.
...
def _best_node(nodes: list[dict[str, Any]], target_node: str, target_core: str, hint: str = "") -> dict[str, Any] | None:
    ...
    # fresh LLM IDs. When dataset text hints exist, allow a lower threshold
    ...
    for pat in activation.get("patterns", []) or []:
        ...
        for key in ("any", "all"):
            for value in pat.get(key, []) or []:
                ...
                if value:
                    terms.add(value)
    trigger_object = activation.get("trigger_object") or {}
    if isinstance(trigger_object, dict):
        for key in ("surface_forms", "target_terms", "state_terms", "action_terms", "semantic_equivalents"):
            for value in _string_list(trigger_object.get(key)):
                terms.add(value)
        for group in trigger_object.get("slot_groups") or []:
...
def _deconflict_activation_only_evidence(group: dict[str, Any], activation_terms: set[str]) -> None:
    ...
    group["patterns"] = _normalize_pattern_list(group.get("patterns"), default_speaker="assistant")
    for pat in group.get("patterns", []) or []:
        if not isinstance(pat, dict):
            continue
...
def _add_window_union_patterns(group: dict[str, Any]) -> None:
    ...
    LLM sometimes emits a single evidence pattern with min_any_hits>1 even
...
def _tokens(text: str) -> set[str]:
    ...
    for part in parts:
        if not part:
            continue
        out.add(part)
        # Domain-neutral Chinese segmentation fallback: LLM may name the
